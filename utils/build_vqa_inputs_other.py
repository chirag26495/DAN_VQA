...
def vqa_processing(image_dir, annotation_file, question_file, valid_answer_set, image_set):
    ...
    for n_q, q in enumerate(questions):
        ...
        if qid2ann_dict[question_id]['answer_type'] != 'other':
            continue
...
def main(args):
    ...
    np.save(args.output_dir+'/train_other.npy', np.array(train), allow_pickle=True)
    np.save(args.output_dir+'/valid_other.npy', np.array(valid), allow_pickle=True)
    # np.save(args.output_dir+'/train_valid_other.npy', np.array(train+valid), allow_pickle=True)
    # np.save(args.output_dir+'/test_other.npy', np.array(test), allow_pickle=True)
    # np.save(args.output_dir+'/test-dev_other.npy', np.array(test_dev), allow_pickle=True)
