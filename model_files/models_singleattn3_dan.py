...
class ImgAttentionEncoder(nn.Module):
    ...
    def __init__(self, embed_size, fine_tuning=False):
        ...
        ### fine-tuning this gives best results
...
class SANModel(nn.Module):
    ...
    def __init__(self, embed_size, qst_vocab_size, ans_vocab_size, word_embed_size, num_layers, hidden_size): 
        ...
        self.qst_encoder = QstEncoder(qst_vocab_size, word_embed_size, embed_size, num_layers, hidden_size)
        self.san = Attention(512, embed_size) #nn.ModuleList([Attention(512, embed_size)]*self.num_attention_layer)
        self.tanh = nn.Tanh()
    ...
    def forward(self, img, qst):
        ...
        return combined_feature, attn_scores
...
class TripletLoss(nn.Module):
    ...
    def calc_euclidean(self, x1, x2):
        # print(f'x1: {x1.shape}', flush=True)
        # print(f'x1 squeeze: {x1.squeeze().shape}', flush=True)
        # print()

        # return (x1 - x2).pow(2).sum(1)
        return (x1.squeeze() - x2.squeeze()).pow(2).sum(1)
    
    def forward(self, anchor: torch.Tensor, positive: torch.Tensor, negative: torch.Tensor) -> torch.Tensor:
        distance_positive = self.calc_euclidean(anchor, positive)
        distance_negative = self.calc_euclidean(anchor, negative)
        
        # print(f'distance_negative: {distance_negative.shape}', flush=True)
        # print(f'distance_positive: {distance_positive.shape}', flush=True)
        # print()
